...
class _SecretsFilter(logging.Filter):
    """Masque la clé API Jeedom et le mot de passe Okofen dans TOUS les logs, y compris
    ceux des bibliothèques (urllib3 journalise l'URL de callback avec ?apikey=…)."""

    def filter(self, record):
        secrets = [s for s in (_apikey, _password) if s]
        if secrets:
...
def execute_one_write(item, jcom):
    ...
    try:
        ...
        logging.error('[write] erreur %s.%s: %s: %s', component, var, type(e).__name__, e)
...
def execute_one_regen(item, jcom):
    ...
    try:
        ...
        logging.error('[regen] erreur %s: %s: %s', component, type(e).__name__, e)
...
def polling_loop(jcom):
    ...
    while True:
        try:
            ...
            logging.error('[poll] erreur: %s: %s', type(e).__name__, e)
            logging.debug(traceback.format_exc())
            time.sleep(_IDLE_SLEEP)
...
for _handler in logging.getLogger().handlers:
    _handler.addFilter(_SecretsFilter())
