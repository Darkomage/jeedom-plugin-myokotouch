...
class myokotouch extends eqLogic
{
    ...
    public static function syncCommands(myokotouch $eqLogic, array $componentData): void
    {
        ...
        $genericCfg   = $cfg[$okoType]['generic'] ?? [];
        ...
        // Nettoyage : une version antérieure créait à tort une commande INFO pour la variable
        // Okofen 'refresh' (logicalId 'refresh', réservé à la commande d'action « Rafraîchir »
        // de Jeedom) → conflit avec la contrainte unique (eqLogic_id, name). On la retire.
        if (isset($existingByLogicalId['info']['refresh'])) {
            $existingByLogicalId['info']['refresh']->remove();
            unset($existingByLogicalId['info']['refresh']);
        }
        ...
        foreach ($componentData as $key => $varData) {
            ...
            // 'refresh' : logicalId réservé à la commande d'action « Rafraîchir » interne de
            // Jeedom (postSave). Ne pas créer de commande plugin pour la variable Okofen
            // 'refresh' (sinon conflit de nom → violation de la contrainte unique).
            if ($key === 'refresh') {
                continue;
            }
            ...
            self::applyGenericType($infoCmd, $genericCfg[$key] ?? null, $regenMode !== null);
            $infoCmd->save();
            ...
            if (!$isReadOnly && is_array($varData)) {
                ...
                self::applyGenericType($actCmd, $genericCfg[$actLogicalId] ?? null, $regenMode !== null);
    ...
    /**
     * Applique le type générique Jeedom défini dans commands.php ('generic').
     * Ne remplace pas un type déjà présent (réglé à la main par l'utilisateur),
     * sauf en régénération ($force) où, comme le nom, il est réappliqué.
     */
    private static function applyGenericType(cmd $cmd, ?string $genericType, bool $force): void
    {
        if ($genericType === null) {
            return;
        }
        if ($force || (string) $cmd->getGeneric_type() === '') {
    ...
    public static function deamon_start(array $_params = []): void
    {
        ...
        // Log sans secrets : clé API Jeedom et mot de passe Okofen masqués
        $cmdLog = str_replace(
            [escapeshellarg($apiKey), escapeshellarg($cfg['password'])],
            ["'***'", "'***'"],
            $cmd
        );
        log::add('myokotouch', 'info', '[deamon_start] lancement: ' . $cmdLog);
    ...
    public static function deamon_stop(): void
    {
        ...
        if (file_exists($pidFile)) {
            ...
            if ($pid > 0) {
                ...
                // 15 = SIGTERM. La constante SIGTERM vient de l'extension pcntl, absente du PHP
                // web (Apache/FPM) sous Debian : en PHP 8 elle lève une Error fatale.
                posix_kill($pid, 15);
                sleep(2);
            }
            unlink($pidFile);
        ...
        exec('pkill -f "myokotouch/resources/demond/demond.py" 2>/dev/null');
        sleep(1);
